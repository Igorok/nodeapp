...
apiBlog.getMyPostDetail = (opts) => {
    ...
    .then(() => {
        return new Promise((resolve, reject) => {
            opts._bId = helper.mongoId(opts._bId);
            db.collection('blogs').findOne({_id: opts._bId}, (e, r) => {
                if (e) return reject(e);
...
apiBlog.editMyPostDetail = (opts) => {
    ...
    if (! opts.name || ! opts.description || ! opts.status || ! opts._bId) {
        return Promise.reject(new Error('Name and description are required'));
    }
    opts._id = opts._id.toString();
    opts._bId = opts._bId.toString();
    opts.status = opts.status.toString();
    opts.name = xss(opts.name.toString().trim());
    opts.description = xss(opts.description.toString().trim());

    if (opts._id === '-1') {
        return apiBlog.addMyPostDetail(opts);
    } else {
        return apiBlog.updateMyPostDetail(opts);
    }
};


apiBlog.updateMyPostDetail = (opts) => {
    let user = null;
    opts._id = helper.mongoId(opts._id);

    let setObj = {
        _bId: helper.mongoId(opts._bId),
        status: opts.status,
        name: opts.name,
...
apiBlog.addMyPostDetail = (opts) => {
    ...
    .then((u) => {
        ...
        setObj.uId = user._id;
        return new Promise((resolve, reject) => {
            db.collection('posts').insertOne(setObj, (e) => {
                if (e) return reject(e);
                resolve();
            });
        });
